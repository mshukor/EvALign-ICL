...
class CHAIR(object):
    ...
    def __init__(self, imids, coco_path, synonym_path=None):
        ...
        #read in synonyms
        synonyms = open(synonym_path).readlines()
    ...
    def _load_generated_captions_into_evaluator(self, cap_file):
        ...
        '''
        #Read in captions        
        self.caps, imids = load_generated_captions(cap_file) # self.metrics
        try:
            assert imids == set(self.imid_to_objects.keys())
        except:
            print("assert imids == set(self.imid_to_objects.keys())")
            pass
    ...
    def compute_chair(self, cap_file, verbose=False):
