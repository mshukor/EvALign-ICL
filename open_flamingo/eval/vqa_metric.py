...
class VQA:
    ...
    def loadRes(self, resFile, quesFile):
        ...
        for ann in anns:
            ...
            ann["answer_type"] = qaAnn.get("answer_type", qaAnn["question_type"])
            # print(ann["answer_type"])
    ...
    def evaluate(self, quesIds=None):
        ...
        for quesId in quesIds:
            ...
            for gtAnsDatum in gts[quesId]["answers"]:
                ...
                if len(otherGTAns) == 0: # in case of one gt answer
            ...
            ansType = gts[quesId].get("answer_type", gts[quesId]["question_type"])
...
def compute_vqa_accuracy(result_json_path, question_json_path=None, annotation_json_path=None):
    ...
    """
    vqaEval.evaluate()
    print(vqaEval.accuracy)
    return vqaEval.accuracy["overall"]




def f1_score(predicted_labels, actual_labels, P_label='absurd', N_label='valid'):
    # Calculate True Positives (TP), False Positives (FP), False Negatives (FN)
    
    TP = sum(1 for pred, actual in zip(predicted_labels, actual_labels) if pred == actual == P_label)
    FP = sum(1 for pred, actual in zip(predicted_labels, actual_labels) if pred == P_label and actual == N_label)
    FN = sum(1 for pred, actual in zip(predicted_labels, actual_labels) if pred == N_label and actual == P_label)
