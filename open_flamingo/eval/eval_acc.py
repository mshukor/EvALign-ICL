...
def main():
    ...
    flamingo, image_processor, tokenizer = create_model_and_transforms(
        ...
        cross_attn_every_n_layers=args.cross_attn_every_n_layers, low_cpu=args.low_cpu, 
        cache_dir=args.cache_dir, use_local_files=not args.skip_use_local_files
    )
...
def evaluate_coco_flickr(
    ...
    for batch in more_itertools.chunked(tqdm(eval_dataset, desc=desc), batch_size):
        ...
        if mode is None:
            ...
            context_text = [
                get_context_text(
                    ...
                    num_shots=num_shots, instruction=instruction
                )
                for i in range(len(batch))
            ]
            ...
            postprocess_captioning_generation = caption_utils.postprocess_generation
            ...
            postprocess_captioning_generation_ = partial(postprocess_generation, objects_first=objects_first, objects_prompt=objects_prompt)
            ...
            batch_text = [f"{context_text[i]}<image>{test_prompt}" for i in range(len(batch))]


            postprocess_captioning_generation = postprocess_captioning_generation_
        ...
        if is_llava:
            ...
            [
                {"text": p, "question_id": sample["image_id"], "answer_id": shortuuid.uuid(), "model_id": "OF", "metadata": {}, "prompt": sample["question"]}
...
def evaluate_itm(
    ...
    for batch in more_itertools.chunked(tqdm(eval_dataset, desc=desc), batch_size):
        ...
        if 'itm' in mode and "objects" in mode:
            ...
            batch_text = [context_text[i] + prompt_function(s, train=False, label=l) for i, (s, l) in enumerate(zip(batch, labels))]

            post_prompt = prompt.split(" ")[0]
            postprocess_captioning_generation = partial(itm_utils.postprocess_generation_itm_objects, prompt=post_prompt)
            ...
            postprocess_captioning_generation = itm_utils.postprocess_generation_itm
            ...
            batch_text = [context_text[i] + itm_utils.get_prompt(s, train=False, label=l) for i, (s, l) in enumerate(zip(batch, labels))]


            postprocess_captioning_generation = itm_utils.postprocess_generation
        ...
        new_predictions = [
            postprocess_captioning_generation(out).replace('"', "").replace(")", "").replace(".", "").strip()
            for out in tokenizer.batch_decode(outputs, skip_special_tokens=True)
        ]
    ...
    for batch in more_itertools.chunked(
        ...
        if mode is not None:
            ...
            if 'explain' in mode:
                ...
                if 'explainonly' in mode:
                    ...
                    get_prompt_explain_ = partial(vqa_utils.get_prompt_explain, onlyexplain=True, exp_prompt=exp_prompt, 
                ...
                context_text = [
                    get_context_text(
                        ...
                        in_context_samples=batch_demo_samples[i],
                        effective_num_shots=effective_num_shots,
                        num_shots=num_shots, text_prompt=text_prompt,
                        neg_samples=in_context_samples,
                        mode=mode,
                ...
                batch_text = [
                    context_text[i] + get_prompt_explain_(s, train=False) for i, s in enumerate(batch)
                ]
                ...
                context_text = [
                    get_context_text(
                        ...
                        in_context_samples=batch_demo_samples[i],
                        effective_num_shots=effective_num_shots,
                        num_shots=num_shots, text_prompt=text_prompt, instruction=instruction
                    )
                    for i in range(len(batch))
                ]
                ...
                context_text = [
                    get_context_text(
                        ...
                        in_context_samples=batch_demo_samples[i],
                        effective_num_shots=effective_num_shots,
                        num_shots=num_shots, text_prompt=text_prompt, mode=mode, instruction=instruction
                    )
                    for i in range(len(batch))
                ]

                if num_shots == 1: # for zero-shot without context
        ...
        if postprocess_generation_exp:
            ...
            predicted_exp.extend([
                postprocess_generation_exp(out)
                for out in tokenizer.batch_decode(outputs, skip_special_tokens=True)])
        ...
        if postprocess_generation_unans:
            ...
            predicted_exp.extend([
                postprocess_generation_unans(out).strip()
                for out in tokenizer.batch_decode(outputs, skip_special_tokens=True)])
